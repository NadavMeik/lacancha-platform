import type { MatchLineups, LineupPlayer, LineupRole } from '../types/lineup'

function xi(rows: Array<[string, string, number, LineupRole]>): LineupPlayer[] {
  return rows.map(([playerId, name, number, role]) => ({
    playerId,
    name,
    number,
    role,
...
export const lineups: Record<string, MatchLineups> = {
